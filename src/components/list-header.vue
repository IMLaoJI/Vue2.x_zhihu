<template>
    ...
    <div class="header-icon" @click="showBar"><i class="fa fa-align-justify fa-2x"></i></div>
    <div v-if="!tip" class="header-cont"><p>{{title}}</p></div>
    <div v-if="tip" class="header-cont"><p>{{tip | dateTime}}</p></div>
    <div class="header-icon" v-show="iconDisplay" ><i class="fa  fa-bullhorn fa-2x"></i></div>
    <div class="header-icon" v-show="iconDisplay" @click="changeMode"><i class="fa fa-adjust fa-2x"></i></div>
  </div>
...
//      window.document.querySelector('html').style.overflow = 'hidden'
        // this.showSidebar1 = !this.showSidebar
         this.$emit("show-menu-slide")
      },
      changeMode () {
        if (!this.nightStyle) {
