...
<template>
  <div>
    <!--详情页头部-->
    <detail-header  :popularity="extra.popularity" :comments="extra.comments" @show-share="showShare1"></detail-header>
    ...
    <div class="detail-main-box">
      ...
      <!--推荐者部分-->
      <div v-if="recommenders.length > 0" class="recommenders-box">
        <router-link :to="{path: '/recommenders' }">   
         <p>推荐者</p>
      ...
      <div v-if="section.name" class="section-box" >
...
beforeDestroy(){
    let _this = this
   console.log(_this.jiyi);
     window.sessionStorage.detailId =  _this.jiyi
...
//          url: 'http://news-at.zhihu.com/api/4/news/' + _this.$route.params.id,
          url: `${url}/api/4/news/` + _this.$route.params.id,
          method: 'GET',
          callback: function (res) {
            _this.body = res.body;
            _this.image = res.image;
            _this.title = res.title;
            _this.imageSource = res.image_source;
           
            
            if (res.recommenders) {
               _this.recommenders = res.recommenders;
            }
            if(_this.body!=''){
              Indicator.close();
            }
            if (res.section) {
              ...
              _this.thumbnail =  res.section.thumbnail;           
            }
            if (!res.body) {
              _this.shareUrl =  res.share_url;
...
//          url: 'http://news-at.zhihu.com/api/4/story-extra/' + _this.$route.params.id,
          url:`${url}/api/4/story-extra/` + _this.$route.params.id,
          method: 'GET',
          callback: function (res) {
            _this.extra = res;
