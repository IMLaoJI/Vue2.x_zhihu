...
<template>
  <div class="test">
    <div class="comments-header">
     ...
     <div class="header-icon"><i class="iconfont">&#xe627</i><span></span></div>
    </div>
  ...
  export default{
    ...
    beforeRouteEnter (to, from, next) {
      ...
      next(vm => {
            ...
            vm.detailId = window.sessionStorage.detailId;
            console.log(vm.detailId);
            vm.extra = JSON.parse(window.sessionStorage.extra);
            Indicator.open({
          text: 'Loading...',
...
//          url: 'http://news-at.zhihu.com/api/4/story/' + _this.detailId + '/long-comments',
          url: `${url}/api/4/story/` + _this.detailId + '/long-comments',
          method: 'GET',
          callback: function (res) {
            _this.longComments = res.comments;
            Indicator.close();
...
//          url: 'http://news-at.zhihu.com/api/4/story/' + _this.detailId + '/short-comments',
          url: `${url}/api/4/story/`+ _this.detailId + '/short-comments',
          method: 'GET',
          callback: function (res) {
            console.log(res);
            _this.shortComments = res.comments
           
              Indicator.close();
            
           _this.id  =  _this.shortComments[_this.shortComments.length - 1].id;
      ...
      getShortCommentsNext () {
        ...
        ajax({
          url: `${url}/api/4/story/` + _this.detailId + '/short-comments/before/' + _this.id,
          method: 'GET',
          callback: function (res) {
             Indicator.close();
            _this.shortComments = _this.shortComments.concat(res.comments)
            _this.id = _this.shortComments[_this.shortComments.length - 1].id
